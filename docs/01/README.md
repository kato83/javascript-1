...
VSCodeの操作は授業を通して慣れてもらいつつ、実際にJavaScriptのコードを書いていきましょう。
プログラミングは「書いて」「実行して」「結果を確認する」の繰り返しです。

## JavaScriptファイルの作成

JavaScriptのコードは `.js` という拡張子のファイルに書きます。

### ファイル作成手順

1. VSCodeで `Ctrl + N` を押して新規ファイルを作成
2. `Ctrl + S` で保存ダイアログを開く
3. 保存先を `docs/01/work` フォルダに設定
4. ファイル名を `hello.js` として保存

> [!TIP]
> ファイル名は半角英数字とアンダースコア（_）、ハイフン（-）を使うのが一般的です。
> スペースや日本語は避けましょう。

## コンソール出力

プログラミングの第一歩は、画面に何かを表示することから始まります。
JavaScriptでは `console.log()` を使って、ターミナルに文字を表示できます。

### 基本的な出力方法

`hello.js` ファイルに以下のコードを書いてみましょう：
...
- `//` から始まる行は **コメント** です。プログラムの動作には影響しません。
- `console.log()` は **関数** と呼ばれるもので、括弧 `()` の中に書いた内容を表示します。
- `"こんにちは、JavaScript!"` は **文字列** と呼ばれ、ダブルクォーテーション `"` またはシングルクォーテーション `'`で囲みます。
- 行末のセミコロン `;` は文の区切りを表します。
...
```
deno run ./docs/01/work/hello.js
```

3. ターミナルに「こんにちは、JavaScript!」と表示されれば成功です！

> [!NOTE]
> コマンドを実行する前に、必ずファイルを保存（`Ctrl + S`）してください。
> 保存していないと変更が反映されません。

## console（コンソール）オブジェクトとは

JavaScriptの `console` は、デバッグ情報を出力するための特別な **オブジェクト** です。
プログラムの動作確認や問題解決に役立ちます。

### 主なconsoleの使い方

`hello.js` に以下のコードを追加して試してみましょう：

```js
// 通常のログ出力
console.log("これは普通のログです");

// 警告メッセージ
console.warn("これは警告メッセージです");

// エラーメッセージ
// TIP: ログの重大度毎に色を分けたい場合は以下を参考に指定が必要
// https://docs.deno.com/examples/color_logging/
console.error("%cこれはエラーメッセージです", "color: red");
...
実行すると、関数に渡したメッセージが表示されます。
`console.log(...)` 等に渡す引数を工夫することで出力されるメッセージの色などをカスタマイズすることも可能です。

### 複数の値を出力する

`console.log()` は複数の値をカンマ区切りで渡すことができます：

```js
console.log("名前:", "山田太郎", "年齢:", 20);
```

### 変数の内容を確認する

プログラム内の値（変数）を確認するのにも使えます：
...
`docs/01/work/introduction.js` という新しいファイルを作成し、以下の内容を表示するプログラムを書いてみましょう：
...
console.log("===== 自己紹介 =====");
console.log("名前: 山田太郎");
console.log("好きな食べ物: ラーメン");
console.log("趣味: サッカー、読書");
console.log("===================");
```

実行して、正しく表示されるか確認してみましょう。

## 計算結果を表示する

JavaScriptは計算もできます。計算結果をconsole.logで表示してみましょう。
...
JavaScriptでは、`+` 記号を使って文字列を連結することもできます。
...
console.log("計算結果: " + (10 + 5)); // 括弧を使うと計算が優先される
```

## JavaScriptの基本的な特徴

JavaScriptには、初心者が知っておくと便利な特徴がいくつかあります。

### コメント

コードの説明や一時的に無効にしたいコードには、コメントを使います：

```js
// 一行コメント - この行はプログラムとして実行されません
...
JavaScriptでは、文の終わりにセミコロン（`;`）をつけるのが一般的です：
...
> [!NOTE]
> JavaScriptではセミコロンを省略することもできますが、
> 初心者のうちは必ずつける習慣をつけましょう。

### エラーメッセージの読み方

プログラムを実行して、エラーが出たときは慌てずにメッセージを読みましょう。
エラーメッセージには、何が問題かのヒントが書かれています。
...
```
error: The module's source code could not be parsed: Unexpected eof at file:[ファイルパス]:[ファイルの行番号]:[文字目]
```

これは「ソースコードを解析して実行することができない」という意味です。

また、エディタ上で `閉じ忘れ` の `れ` 部分にカーソルを合わせると以下のように表示されます：

```
Unterminated string literal.deno-ts(1002)
```

これは「ダブルクォーテーション `"` が閉じられていない」ことを示しています。

## 実習課題：計算プログラム

`docs/01/work/calc.js` という新しいファイルを作成し、以下の計算をして結果を表示するプログラムを書いてみましょう：

1. 123 + 456 の計算結果
2. 10 × 20 の計算結果
3. 30 ÷ 4 の計算結果
4. 「合計: 」という文字列と、上記3つの計算結果の合計値
...
console.log("123 + 456 =", result1);
console.log("10 × 20 =", result2);
console.log("30 ÷ 4 =", result3);
...
1. JavaScriptファイルの作成方法
2. console.logを使った出力方法
3. 文字列と数値の扱い方
4. 簡単な計算の方法

次の章では、変数とデータ型について詳しく学んでいきます。

> [!IMPORTANT]
> プログラミングは実際に手を動かして書くことが大切です。
> 例題をただ読むだけでなく、必ず自分でコードを書いて実行してみましょう。
