...
// ドット `.` の手前に `?` を付けた `?.` を用いたより簡潔な確認方法もある（オプショナルチェーニングと呼ばれる）
...
1. メモリ機能付き電卓オブジェクト`calculator`を作成し、エクスポートする（以下のプロパティとメソッドを含む）：
   - プロパティ: 
     - `result`（計算結果を保存する数値、初期値は0）
     - `memory`（メモリに保存された数値、初期値は0）
   - 基本計算メソッド:
     - `add(number)`: 数値を加算し、結果を返す
     - `subtract(number)`: 数値を減算し、結果を返す
     - `multiply(number)`: 数値を乗算し、結果を返す
     - `divide(number)`: 数値で除算し、結果を返す（0で割る場合は計算を行わない）
     - `clear()`: 結果を0にリセットし、結果を返す
     - `getResult()`: 現在の結果を返す
   - メモリ機能メソッド:
     - `memoryStore()`: 現在の結果をメモリに保存する
     - `memoryRecall()`: メモリの値を取得する
     - `memoryClear()`: メモリを0にクリアする
     - `memoryAdd()`: 現在の結果をメモリの値に加算してメモリに保存する
     - `memorySubtract()`: 現在の結果をメモリの値から減算してメモリに保存する
